...
non_negativity = F, R, C >= 0

for i in range(0,):
    if gold_constraint and lumber_constraint and food_constraint and non_negativity:
        F = 
...
def Collatz(start_number):
    ...
    return {"collatz_sequence": collatz_sequence, "collatz_length": len(collatz_sequence), "max_collatz": max(collatz_sequence), "sequence_sum": sum(collatz_sequence)} 
...
def Collatz_winner(number_list):
    ...
    for i in number_list:
        ...
        result = {"collatz_length": len(collatz_sequence), "max_collatz": max(collatz_sequence)} 

        all_results.append(result)

    # winner_cl = max(all_results, key=lambda x:x["collatz_length"])
    # winner_mc = max(all_results, key= lambda x:x["max_collatz"])

    # if all_results.index(winner_cl) == all_results.index(winner_mc):
    #     winner = all_results.index(winner_cl) - 1
    # else:
    #     winner = "None" 
